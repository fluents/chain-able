...
function getCall() {
  ...
  _.each(params, function(param) {
    ...
    var parentIndex = parentParam == null
      ? -1
...
function getReturns() {
  ...
  var desc = _.toString(_.get(tag, 'description'))
  var type = _.toString(_.get(tag, 'type.name')) || '*'

  var returns = []
  if (!tag) {
    tag = getTag(this, 'return')
    desc = _.toString(_.get(tag, 'description'))
    type = _.toString(_.get(tag, 'type.name')) || '*'
  }

  return tag ? [type, desc] : returns
}
